---
title: "TRUST4 SHM analysis - WT vs KO project"
output:
  html_document:
    code_download: yes
    code_folding: hide
...
path_data_root <- file.path("/mnt/volume/Projects/lucas_trust4/results_WT_KO") # operating system (Linux, Windows, Mac)
...
bcr_data_filt <- bcr_data %>%
             ...
             dplyr::mutate(mu_freq = as.numeric(mu_freq),
                           group = ifelse(grepl("WT",sample_id),"WT","KO"),
...
bcr_data_filt$sample_id <- factor(bcr_data_filt$sample_id, 
                                     levels=c("WT1","WT2","WT4","WT5",
                                              "KO1","KO3","KO4","KO5"))
bcr_data_filt$group <- factor(bcr_data_filt$group, levels=c("WT","KO"))
...
count_codon <- bcr_data_filt_IGHV172 %>%
  ...
  summarize(count_W = sum(ighv172_codon38_aa == "W"),
            ...
            count_other = sum(!ighv172_codon38_aa %in% c("W", "L" ,"X")))
kable(count_codon)
```
...
perc_codon <- bcr_data_filt_IGHV172 %>%
  ...
  summarize(perc_W = sum(ighv172_codon38_aa == "W")/n() * 100,
            ...
            perc_other = sum(!ighv172_codon38_aa %in% c("W", "L" ,"X"))/n() * 100)
kable(perc_codon)
```
...
ggplot(bcr_data_filt_IGHV172, aes(x=sample_id, fill=ighv172_codon38_aa)) +
  geom_bar() +
  scale_fill_manual(values = c("W" = "slateblue", "L" = "firebrick", "X" = "gray80", "C" = "gray60", "R" = "gray40", "S" = "gray20")) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
### Compare %W38L across all mice

Calculate percentage of W38L from all IGHV1-72 sequences with known aa at codon 38:

```{r fig.width=4, fig.height=3}

perc_W_L_ofknown <- bcr_data_filt_IGHV172 %>%
  dplyr::filter(!ighv172_codon38_aa %in% c("X")) %>%
  dplyr::group_by(sample_id, group) %>%
  dplyr::summarize(perc_W = sum(ighv172_codon38_aa == "W")/n() * 100,
            perc_L = sum(ighv172_codon38_aa == "L")/n() * 100)

ggplot(perc_W_L_ofknown, aes(x=group, y=perc_L, fill=group)) +
  geom_boxplot() +
  geom_point(position = position_dodge(width = 0.75)) +
  scale_y_continuous(limits = c(0, 85)) +
  stat_compare_means(comparisons = list(c("WT","KO")), method = "wilcox.test", label = "p.signif", size=5) +
  labs(x="", y="% W38L", fill="Group")
